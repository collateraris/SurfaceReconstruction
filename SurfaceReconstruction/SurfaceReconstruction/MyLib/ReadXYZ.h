...
namespace my_sr_lib
{
	...
	template<typename T>
	void read_xyz(const char* filename, std::vector<my_sr_lib::SPointXYZ<T>>& _points, SMinMaxPoint<T>& _minmax);
	...
	void read_xyz(const char* filename, std::vector<my_sr_lib::SPointXYZ<T>>& _points, SMinMaxPoint<T>& _minmax)
	{
		...
		while (!in.eof()) {
			...
			T x_ = static_cast<T>(num[0]);
			T y_ = static_cast<T>(num[1]);
			T z_ = static_cast<T>(num[2]);
